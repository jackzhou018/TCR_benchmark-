...
\subsection{2.9 A composite single-prediction confidence
...
interface score of Section 2.8; and $\mathrm{pLDDT}_{\mathrm{CDR3}\alpha}$
...
complexes, and the four-term composite of Section 2.9 --- computed from
...
(0.624--0.803) (Figure 4A).

It is the best of the scores available for an AlphaFold 3 prediction
(Figure 4B). On the same 126 complexes the published random-forest quality-tier
...
no better than the two confidence scores it was built to improve on. It is not, however,
the best-calibrated confidence signal on this benchmark: ESMFold2's own
ipTM orders ESMFold2's predictions better (r = 0.793, \ensuremath{\rho} = 0.777,
Section 3.7) than any AlphaFold 3-derived score orders AlphaFold 3's.
The two are not like for like --- each score is judged on the
predictions of the model that produced it, and no equivalent composite
was built for ESMFold2 --- but the composite buys AlphaFold 3 part, not
all, of the calibration ESMFold2 has without one. The
...
That margin narrows on every subset (Figure 4C). On the 111 class I
...
whiskers are 95\% bootstrap intervals. (\textbf{C}) Paired MOGGER-minus
random-forest difference in each correlation, by subset; whiskers are
95\% paired-bootstrap intervals, and only the full benchmark excludes
zero.}
\end{figure}

\subsection{3.9 Effect of similarity to pre-cutoff
...
accuracy (Figure 5A). Cross-model C\ensuremath{\alpha} RMSD correlates with AlphaFold 3's
...
By divergence quartile (Figure 5B), median Global DockQ falls from 0.860
...
The residual complementarity is nonetheless substantial (Figure 5C). At
...
the equality of outcome conceals very different dynamics (Figure 6A).
...
The same behaviour at scale (Figure 6B, 30 complexes \ensuremath{\times} 5 samples =
...
free to remove. They are (Figure 6C, Table 3). Over the same 30
...
quality-control columns, the sampler-economy tracks with per-complex
timings and paired differences, and the per-structure confidence features
and bootstrap intervals behind the composite score of Section 2.9. All analyses are reproducible from the
