...
\maketitle
\begin{abstract}
T cell receptor (TCR) recognition of peptide--MHC (pMHC) is the central
specificity event of cellular immunity, and structure prediction is now
the main route to modelling it at scale. Published benchmarks agree that
...
class II) carry a scoreable experimental reference. All methods received
byte-identical truncated inputs and a single fixed chain convention, and
every model was scored against natives rebuilt from the biological
assembly with an explicit identity chain mapping. AlphaFold 3 (median
Global DockQ 0.751) and ESMFold2, a single-sequence diffusion predictor
conditioned on a 6-billion-parameter language model rather than on an
MSA (0.736), were statistically indistinguishable (paired Wilcoxon p =
0.68); both exceeded Protenix (0.701, p = 0.045 and p = 0.0009) and the
TCR-specialised TCRmodel2 (0.708 on the 111 class I targets) was not
separable from either. Equal accuracy did not mean equivalent behaviour.
ESMFold2's ipTM tracked accuracy far more tightly than AlphaFold 3's
(Pearson r = 0.79 vs 0.66); AlphaFold 3 was more sensitive to reference
resolution; and the two models placed the TCR more than 90\ensuremath{^{\circ}} apart on 10
of 126 complexes while agreeing on the pMHC to within 0.44 \AA{}. The C\ensuremath{\alpha}
RMSD between the two predictions --- a quantity computable without any
experimental structure --- predicted the worse of the two DockQ scores
better than either model's own confidence (Spearman \ensuremath{\rho} = \ensuremath{-}0.884 vs +0.60
and +0.64), and the two methods together reached medium-or-better
accuracy on 119 of 126 complexes against 107 and 115 individually.
Instrumenting both samplers showed the equal accuracy is reached by
different routes: AlphaFold 3 is still 44 \AA{} from its answer at the
midpoint of denoising and converges only in the final fifth of its
schedule, whereas ESMFold2 is within 6 \AA{} at its midpoint. Exploiting
that, cutting AlphaFold 3 from 200 to 160 denoising steps and from 5
diffusion samples to 1 gave a 1.80\ensuremath{\times} speed-up on 30 complexes at a cost
of \ensuremath{-}0.010 Global DockQ (95\% CI \ensuremath{-}0.032 to +0.007, p = 0.33). We conclude
that MSAs are not required for state-of-the-art TCR--pMHC modelling,
that cross-architecture agreement is a strong reference-free quality
signal, and that the default AlphaFold 3 sampler is substantially
over-provisioned for complexes of this size.
\end{abstract}
...
Here we assemble a benchmark of 136 TCR--pMHC entries released after 30
September 2021, the training cutoff shared by AlphaFold 3, Protenix and
ESMFold2, of which 126 carry a scoreable experimental reference. Every
method receives identical truncated inputs under one chain convention,
and every prediction is scored against a native rebuilt from the correct
biological assembly. We report interface accuracy (DockQ), TCR-specific
accuracy (MHC-aligned weighted all-CDR C\ensuremath{\alpha} RMSD and framework-aligned
CDR3 all-atom RMSD), confidence calibration, sensitivity to reference
resolution and to training-set similarity, the structural anatomy of the
disagreement between the two leading models, the internal trajectories
of both diffusion samplers, and the wall-clock cost of the default
AlphaFold 3 sampler settings.
...
Candidate entries were drawn from the TCR3d database {[}1,3{]}, which
curates all known TCR complex structures and is the source used by the
most closely related benchmark {[}10{]}. Restricting to \ensuremath{\alpha}\ensuremath{\beta} TCR complexes
...
TCR-mimic antibody complexes, or otherwise incomplete triads and were
not carried forward.

All 136 entries were manually reviewed against their deposited
coordinates and primary publications, and nine were flagged for review
and excluded from scoring for one of two reasons. Five entries carry
non-standard peptide chemistry: 8TRL, 8TRQ and 8TRR each contain one
citrulline and 9NIG contains two, while 9WNT is a lipopeptide whose
attached lipid cannot be represented in a polymer sequence. Because
standard FASTA cannot encode these modified residues, and because the
modification is in every case a determinant of the interaction being
modelled, sequence-input prediction of these complexes is not well
posed. The remaining four entries are covalently tethered constructs in
which the peptide is fused to an MHC-containing construct through an
engineered linker: 8VQ8 and 9AUD carry a peptide--linker--MHC-II\ensuremath{\beta} fusion
(QVYSLIRPNENPAHK--GSGGSIEGRGGSGASGDS--MHC-II\ensuremath{\beta} in 8VQ8), 8VD0 carries a
comparable peptide linker, and 9C3E is a 593-residue single-chain
construct containing peptide, linkers, \ensuremath{\beta}2-microglobulin and MHC-I.
Removing the linker and modelling the components as separate chains
would alter the experimental connectivity and confound the accuracy
comparison.

One further entry, 9J4S, was excluded at the reference-structure stage
(Section 2.3), leaving 126 scoreable complexes, 111 class I and 15 class
II.
...
covariate, and the full threshold sweep is reported so that any reader
who prefers a filtered view can read it off directly.

\subsection{2.2 Domain truncation and chain
convention}\label{domain-truncation-and-chain-convention}

Each complex was truncated to the domains that constitute the TCR--pMHC
...
including residues annotated as experimentally unobserved; accuracy
calculations were restricted to residues with deposited atomic
coordinates, and no missing coordinates were imputed.

A single chain convention was used for every method, with no per-model
remapping (Table 1).

\begin{longtable}[]{@{}lcc@{}}
\caption{\textbf{Benchmark chain convention.} Chain letters are used
verbatim in every method's input and output; no per-model remapping is
performed anywhere in the pipeline. \ensuremath{\beta}2-microglobulin is
excluded.}\tabularnewline
\toprule\noalign{}
Role & Class I & Class II \\
\midrule\noalign{}
\endfirsthead
\toprule\noalign{}
Role & Class I & Class II \\
...
\bottomrule\noalign{}
\endlastfoot
MHC-I \ensuremath{\alpha}1\ensuremath{\alpha}2 / MHC-II \ensuremath{\alpha}1 & A & A \\
Peptide & B & B \\
MHC-II \ensuremath{\beta}1 & --- & C \\
TCR \ensuremath{\alpha} variable & C & D \\
TCR \ensuremath{\beta} variable & D & E \\
\end{longtable}
...
Class I predictions therefore carry chains A,B,C,D and class II
A,B,C,D,E. Because downstream interface scoring assumes chain letters
mean the same thing in every model's output, an automated check
confirmed that the residues present in every prediction CIF matched the
input FASTAs for all methods before scoring.

\subsection{2.3 Reference structures}\label{reference-structures}

Experimental references were rebuilt from RCSB rather than used as
deposited. For each entry, the chains named in the per-entry chain
manifest were extracted and cropped to the benchmark FASTA span, with a
fallback that searches every chain when the manifest's author chain is
wrong (7Z50's peptide is deposited as chain T, not B). Alternate
locations were resolved deterministically to the highest-occupancy
conformer, with altloc A as tie-breaker.

Cutting the right sequence is not sufficient --- it must come from the
right copy. Forty-six of the 136 entries are built from files holding
several complete complexes, and the deposited asymmetric unit is
frequently not a complex at all. Each rebuilt native was therefore
measured for its own interfaces and repaired where necessary by one of
two routes: fetching the author-defined biological assembly
(\texttt{\textless{}PDB\textgreater{}-assembly\textless{}N\textgreater{}.cif},
ranking candidates by full-length chains, then a paired TCR, then
peptide contact, then interface size), or re-imaging coordinates wrapped
across the periodic boundary by sliding a disconnected group by whole
unit cells. Six natives depend on these repairs (7L1D, 7RRG, 7SU9, 7Z50,
9K2I, 9LLU); 7SU9 requires a single \ensuremath{-}a translation, which is not a
biological-assembly operator and therefore appears in no assembly file.

An automated audit checked four properties of every native: crop
exactness against the FASTA, one connected complex, no better-contacting
alternative chain combination, and all chains drawn from a single
author-defined assembly. The last test is the strict one --- a
contact-count test alone cannot distinguish a crystal lattice contact
between two copies from a real interface. It identified 9J4S, for which
neither author-defined assembly contains any contact between the TCR
variable domains and the pMHC; the apparent interface obtained by mixing
chains from different assemblies is a lattice contact. 9J4S was
excluded, and a DockQ score is never computed for it.

Four natives (9EJG, 9EJH, 9EJI, 9LLU) have a TCR that is docked on the
MHC but not touching the peptide. This is biology --- 9EJI is a
published peptide-independent binder --- and these entries were
retained.
...
ESMFold2 (the \texttt{biohub/ESMFold2} checkpoint, revision
\texttt{8fc3ff47}, conditioned on the \texttt{biohub/ESMC-6B} language
model, revision \texttt{45b0fa5d}) {[}21{]} was run through the ESM SDK
