...
complexes remains slow relative to the size of the repertoire --- the
structural databases that curate them hold only a few hundred ternary
complexes [1--3] --- so computational modelling has become the practical
route to structural hypotheses at scale, whether for epitope-specificity
prediction, TCR engineering, or immunotherapeutic design.
...
used by Woods et al. [19]; ``all CDRs'' means all four regions on both
...
AlphaFold 2's pLDDT head was trained to predict [4] and requires no
...
was taken from its output. Per-residue predicted lDDT (pLDDT), the residue-level
confidence introduced with AlphaFold 2 [4], was read from the B-factor column of the same top-ranked prediction file that was
...
quality better than ipTM or pTM [10]. Each term lies on the unit interval, so the
...
against CDR3 RMSD [10]. Pooling CDR3\ensuremath{\alpha} and
...
because extra seeds bought more accuracy than extra samples [19].
...
[12] on a larger and more recent set, and agrees with Lu et al.'s
conclusion that the bottleneck is not the peptide in the groove but the
TCR docking orientation [10], and it argues that further
...
analysis is only as complete as TCR3d \ensuremath{\cup} STCRDab [1--3]. AlphaFold 3 and
