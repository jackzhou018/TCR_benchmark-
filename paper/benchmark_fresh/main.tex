...
idiosyncratic remainder (Section 3.9). Instead, similarity to the
...
the TCR to dock correctly, and the term Lu et al.~found to track docking
quality better than ipTM or pTM [25]. Each term lies on the unit interval, so the
...
\subsection{3.8 A composite single-prediction score beats the published
...
classifier [26] reaches r = 0.662 (\ensuremath{\rho} = 0.674), AlphaFold 3's own ipTM
...
\subsection{3.9 What each method takes from the pre-cutoff
...
(Figure 6A--C). Peptide and MHC identity to the closest pre-cutoff
...
(Figure 6E--F). Every method is worst on the 42 targets clean of every
...
\subsection{3.10 Structural divergence between AlphaFold 3 and
...
accuracy (Figure 7A). Cross-model C\ensuremath{\alpha} RMSD correlates with AlphaFold 3's
...
than the composite and random-forest scores of Section 3.8 --- and it
requires no confidence head, no training, and no calibration.

By divergence quartile (Figure 7B), median Global DockQ falls from 0.860
...
The residual complementarity is nonetheless substantial (Figure 7C). At
...
the successful case directly visible (Figure 8). Colour identifies the
...
\subsection{3.11 Denoising trajectories of the two
...
the equality of outcome conceals very different dynamics (Figure 9A).
...
The same behaviour at scale (Figure 9B, 30 complexes \ensuremath{\times} 5 samples =
...
\subsection{3.12 Effect of AlphaFold 3 sampler settings on accuracy and
...
free to remove. They are (Figure 9C, Table 4). Over the same 30
...
better spent on a second architecture, which Section 3.10 shows is worth
...
API, and the trajectory analysis of Section 3.11 used the local
...
composite confidence score of Section 3.8 was developed on this
...
and bootstrap intervals behind the composite score of Section 2.10. The
pre-cutoff audit of Section 3.9 is provided in full --- per-target rules,
